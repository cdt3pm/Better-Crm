...
const db = new MongoService(); // TODO: call method that gets a BackendService 
let tableNames = null;
async function conditionallyCreateTable(name) {
...
(async () => {
    ...
    // Collection Records
    const collectionId = (await conditionallyCreateRecord(new Collection(Collection.COLLECTION_NAME, "Collection", "Collections", collectionAttributes))).newId;
    const attributeId = (await conditionallyCreateRecord(new Collection(Attribute.COLLECTION_NAME, "Attribute", "Attributes", attributeAttributes, true))).newId;
    const relationshipId = (await conditionallyCreateRecord(new Collection(Relationship.COLLECTION_NAME, "Relationship", "Relationships", relationshipAttributes))).newId;
    // Relationship Records
    // TODO: maybe rethink this. Maybe put other half of relationship embedded on collection????
    conditionallyCreateRecord(new Relationship("collection_attribute", collectionId, attributeId, Collection.ATTRIBUTES_ATTRIBUTE, Attribute.NAME_ATTRIBUTE, RelationshipType.oneToMany, EmbeddedType.embeddedDictionary));
    conditionallyCreateRecord(new Relationship("collection_relationship_from", collectionId, relationshipId, null, Relationship.FROM_COLLECTION_ATTRIBUTE, RelationshipType.oneToMany, EmbeddedType.notEmbedded));
    conditionallyCreateRecord(new Relationship("collection_relationship_to", collectionId, relationshipId, null, Relationship.TO_ATTRIBUTE_ATTRIBUTE, RelationshipType.oneToMany, EmbeddedType.notEmbedded));
})();
